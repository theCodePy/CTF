...
def mikeSwift_reverse(sik):
    sto = sik.split("1h4ck")[1:]
    encrypted = ""
    for i in sto:
        encrypted += i[0]
    return encrypted

crypted = mikeSwift_reverse(sik)
print("mikeSwift reversed: \n", crypted)
print()

def crypt_reverse(crypted):
    fusc_rev = crypted[3:]
    rev_crypt = fusc_rev[-1::-1]
    obfuscated = ""
    ptr = 0
    ittr = 0
    while ptr < len(rev_crypt):
        ptr += len(str(ittr))
        obfuscated += rev_crypt[ptr]
        ptr+=1
        ittr +=1
    return obfuscated

obfuscated = crypt_reverse(crypted)
print("Crypt reverse: \n",obfuscated)
print()

def obfuscate_reverse(obfuscated):
    refus = obfuscated.replace("florSFIUEfet4565477", '')
    fusc = refus.replace("534345fdfgfgfdhty6y56yjl", '')
    decoded = base64.b64decode(fusc)
    return decoded

decoded = obfuscate_reverse(obfuscated)
